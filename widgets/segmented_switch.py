"""Reusable N-segment animated switch widget (custom-painted).

Consolidates the duplicated painting/animation logic formerly split
between ``ReviewSegmentedSwitch`` (2 segments) and ``_ModeSwitch``
(3 segments) in restore_mode_selector.py.

Usage::

    switch = SegmentedSwitch(["A", "B", "C"])
    switch.segmentClicked.connect(my_slot)
"""
...
class SegmentedSwitch(QWidget):
    """Custom-painted N-segment switch with sliding cursor animation."""
    ...
    def __init__(self, labels, parent=None):
        ...
        self.setFixedSize(self._SEGMENT_W * self._n_segments, self._HEIGHT)
    ...
    def setCurrentIndex(self, idx: int, animated: bool = False) -> None:
        if self._emitting:
            return
        idx = max(0, min(idx, self._n_segments - 1))
    ...
    def mousePressEvent(self, event) -> None:
        ...
        idx = min(int(x / self._SEGMENT_W), self._n_segments - 1)
        if idx < 0:
            idx = 0
        if idx == self._current:
            return
